...
if __name__ == '__main__':
    ...
    print(f"Tielines:{TieLinesID},{TieLinesName}")
    #try:
    ...
    Vprofile = PFresult.results.get_bus_df().Vm
    runtime = time.time() - start
    print(f"TieLines: The new optimal configuration losses:{loss:.3f}, radiality:{radiality}, fitness_ratio=1, numPF:{dnr.NumPF}, vmax:{Vprofile.max():.3f}, vmin:{Vprofile.min():.3f}, time={runtime:.3f} ") 
    results['original '+caseName] = [caseName, 'TieLines',['TieLines'],loss,Vprofile,runtime, fitness_ratio, radiality, dnr.NumPF, TieLinesName]
    #except:
    ...
    try:
        ...
        Vprofile = PFresult.results.get_bus_df().Vm
        runtime = time.time() - start
        print(f"Merlin: The new optimal configuration losses:{loss:.3f}, radiality:{radiality}, numPF:{dnr.NumPF}, vmax:{Vprofile.max():.3f}, vmin:{Vprofile.min():.3f}, time={runtime:.3f}   ")
        results['Merlin'+caseName] = [caseName, 'Merlin',['TieLines'],loss,Vprofile,runtime, fitness_ratio, radiality, dnr.NumPF, disabled_lines]
    except:
    ...
    try:
        ...
        Vprofile = PFresult.results.get_bus_df().Vm
        runtime = time.time() - start
        print(f"Baran: The new optimal configuration losses:{loss:.3f}, radiality:{radiality}, numPF:{dnr.NumPF}, vmax:{Vprofile.max():.3f}, vmin:{Vprofile.min():.3f}, time={runtime:.3f}   ") 
        results['Baran'+caseName] = [caseName, 'Baran', ['TieLines'],loss,Vprofile,runtime, fitness_ratio, radiality, dnr.NumPF, disabled_lines]
    except:
    ...
    try:
        ...
        Vprofile = PFresult.results.get_bus_df().Vm
        runtime = time.time() - start
        print(f"Salkuti: The new optimal configuration losses:{loss:.3f}, radiality:{radiality}, numPF:{dnr.NumPF}, vmax:{Vprofile.max():.3f}, vmin:{Vprofile.min():.3f}, time={runtime:.3f}   ") 
        results['Salkuti'+caseName] = [caseName, 'Salkuti',['TieLines'],loss,Vprofile,runtime, fitness_ratio, radiality, dnr.NumPF, disabled_lines]
    except:
    ...
    try:
        ...
        Vprofile = PFresult.results.get_bus_df().Vm
        runtime = time.time() - start
        print(f"Montoya/MSTgreedy: The new optimal configuration losses:{loss:.3f}, radiality:{radiality}, numPF:{dnr.NumPF}, vmax:{Vprofile.max():.3f}, vmin:{Vprofile.min():.3f}, time={runtime:.3f}   ") 
        results['Montaya/MSTgreedy'+caseName] = [caseName, 'MSTgreedy', [False,False,True,'prim'],loss,Vprofile,runtime, fitness_ratio, radiality, dnr.NumPF, disabled_lines]
    except:
    ...
    try:
        ...
        if caseName=="118 buses":
            ...
            Vprofile = PFresult.results.get_bus_df().Vm
            runtime = time.time() - start
            print(f"Khalil: The new optimal configuration losses:{loss:.3f}, radiality:{radiality}, numPF:{dnr.NumPF}, vmax:{Vprofile.max():.3f}, vmin:{Vprofile.min():.3f}, time={runtime:.3f}   ")
            results['Khalil'+caseName] = [caseName, 'Khalil', [10,1,0.2],loss,Vprofile,runtime, fitness_ratio, radiality, dnr.NumPF, disabled_lines]
    except:
        print("falla Khalil")
    ...
    try:
        ...
        Vprofile = PFresult.results.get_bus_df().Vm
        runtime = time.time() - start
        print(f"Jakus: The new optimal configuration losses:{loss:.3f}, radiality:{radiality}, numPF:{dnr.NumPF}, vmax:{Vprofile.max():.3f}, vmin:{Vprofile.min():.3f}, time={runtime:.3f}   ")
        results['Jakus'+caseName+str(fitness_ratio)] = [caseName,'Jakus' ,[0.4,16,20,2,'TieLines',1,0.02],loss,Vprofile,runtime, fitness_ratio, radiality, dnr.NumPF, disabled_lines]
    except:
        print("Jakus"+caseName+str(fitness_ratio)+" method has failed")
        

    pd.DataFrame.from_dict(results, orient='index', columns=['case', 'Method','parameters', 'Loss','Vprofile','time','fitness_ratio','Radiality', 'NumPF', 'DisabledLines']).to_excel('.\\data\\results_voltage_profile.xlsx')